...
module.exports = function (grunt) {
    ...
    var author    = grunt.option('author') || 'treborbay';
    var email     = grunt.option('email')  || 'treborbay@t-online.de';
    var fs        = require('fs');
    ...
    if (process.argv[2] == 'rename') {
        ...
        if (!newname) {
            console.log('Please write the new sienna name, like: "grunt rename --name=mywidgetset" --author="Author Name"');
            process.exit();
        }
        ...
        if (fs.existsSync(__dirname + '/admin/sienna.png')) {
            fs.renameSync(__dirname + '/admin/sienna.png',              __dirname + '/admin/' + newname + '.png');
        }
        if (fs.existsSync(__dirname + '/widgets/sienna.html')) {
            fs.renameSync(__dirname + '/widgets/sienna.html',           __dirname + '/widgets/' + newname + '.html');
        }
        if (fs.existsSync(__dirname + '/widgets/sienna/js/sienna.js')) {
            fs.renameSync(__dirname + '/widgets/sienna/js/sienna.js', __dirname + '/widgets/sienna/js/' + newname + '.js');
        }
        if (fs.existsSync(__dirname + '/widgets/sienna')) {
            fs.renameSync(__dirname + '/widgets/sienna',                __dirname + '/widgets/' + newname);
        }
    }
            ...
            name: {
                options: {
                    patterns: [
                        ...
                        {
                            match: /Sienna/g,
                            replacement: newname ? (newname[0].toUpperCase() + newname.substring(1)) : 'Sienna'
                        },
                        {
                            match: /treborbay/g,
                            replacement: author
